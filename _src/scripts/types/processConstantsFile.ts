...
import ensureConstantsNode from './ensureConstantsNode';
import ensureNode from './ensureNode';

function processConstFile(file: string): cytoscape.CollectionReturnValue[] {
  ...
  const [node, edge] = ensureNode({
    id: `${env.zz_scope}/types/${lang}/${pack}/${ver}/constants/${name}`,
    name: name,
    label: name,
    details: data,
    parent: ensureConstantsNode(file)[0],
  });
  node.addClass('composition__file');
  return [node, edge];
}

export default processConstFile;
