...
import ensureEnumsNode from './ensureEnumsNode';
import ensureNode from './ensureNode';

function processEnumsFile(file: string): cytoscape.CollectionReturnValue[] {
  ...
  const [node, edge] = ensureNode({
    id: `${env.zz_scope}/types/${lang}/${pack}/${ver}/enums/${name}`,
    name: name,
    label: name,
    details: data,
    parent: ensureEnumsNode(file)[0],
  });
  node.addClass('composition__file');
  return [node, edge];
}

export default processEnumsFile;
