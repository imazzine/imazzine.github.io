...
import ensureExportsNode from './ensureExportsNode';
import ensureNode from './ensureNode';

function processExportsFile(file: string): cytoscape.CollectionReturnValue[] {
  ...
  const [node, edge] = ensureNode({
    id: `${env.zz_scope}/types/${lang}/${pack}/${ver}/exports/${name}`,
    name: name,
    label: name,
    details: data,
    parent: ensureExportsNode(file)[0],
  });
  node.addClass('composition__file');
  return [node, edge];
}

export default processExportsFile;
