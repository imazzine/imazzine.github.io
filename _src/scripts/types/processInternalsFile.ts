...
import ensureInternalsNode from './ensureInternalsNode';
import ensureNode from './ensureNode';

function processInternalsFile(file: string): cytoscape.CollectionReturnValue[] {
  ...
  const [node, edge] = ensureNode({
    id: `${env.zz_scope}/types/${lang}/${pack}/${ver}/internals/${name}`,
    name: name,
    label: name,
    details: data,
    parent: ensureInternalsNode(file)[0],
  });
  node.addClass('composition__file');
  return [node, edge];
}

export default processInternalsFile;
